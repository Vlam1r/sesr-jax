...
def upscale_image_with_model(low_res_rgb: tf.Tensor, model: models.model.Model, model_weights: Dict, model_mask: Dict) -> tf.Tensor:
    ...
    cb_upsampled = tf.image.resize(cb[..., np.newaxis], (2 * cb.shape[0], 2 * cb.shape[1]), method="bicubic")[np.newaxis, ...]
    cr_upsampled = tf.image.resize(cr[..., np.newaxis], (2 * cr.shape[0], 2 * cr.shape[1]), method="bicubic")[np.newaxis, ...]
    net_input = y.numpy()[np.newaxis, ...][..., np.newaxis] / 255
    y_upsampled = model.apply(model_weights, model_mask, net_input)
